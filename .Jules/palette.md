...
## 2026-04-03 - [Skip-to-content and Scroll-Spy Accessibility]
**Learning:** For skip-to-content links to work reliably across all browsers, the target element (usually `<main>`) must have `tabindex="-1"` and a unique ID. Additionally, scroll-spy implementations should toggle `aria-current="location"` on navigation links to inform screen reader users of their position on the page.
**Action:** Always ensure the main content container has `tabindex="-1"` when targeted by a skip-link, and update scroll-spy logic to include `aria-current` attributes for better screen reader support.
